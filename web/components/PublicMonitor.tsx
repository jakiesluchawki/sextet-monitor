...
import { buildSituation, createBriefingText } from "@/lib/situation";
import { AREAS_STORAGE_KEY, MAX_FAVORITE_COUNTRIES, getScopeLabel, normalizeScopeId, readFavoriteCountries, scopeCountryCode, writeFavoriteCountries, type ScopeId } from "@/lib/areas";
...
const scopes:ScopeId[]=["world","europe","poland"];
...
export default function PublicMonitor(){
  ...
  const [favoriteCountries,setFavoriteCountries]=useState<string[]>([]),[favoriteStorageBlocked,setFavoriteStorageBlocked]=useState(false);
  ...
  const [undatedPageSize,setUndatedPageSize]=useState(PAGE_SIZE);
  ...
  useEffect(()=>{
    ...
    restore();const storage=localStore();setStorageAvailable(Boolean(storage));setPinnedIds(readWatchState(storage).ids);setFavoriteCountries(readFavoriteCountries(storage));setSessionReady(true);
    const sync=(event:StorageEvent)=>{
      ...
      if(event.key===AREAS_STORAGE_KEY || event.key===null){setFavoriteCountries(readFavoriteCountries(localStore()));setFavoriteStorageBlocked(false);}
    };
  ...
  useEffect(()=>{setPageSize(PAGE_SIZE);setUndatedPageSize(PAGE_SIZE);},[scope,hours,category,sourceId,search,onlyPinned,onlyChanges,snapshot]);
  useEffect(()=>{
  ...
  const undatedRecords=useMemo(()=>filterViewEvents(situation?.undatedEvents || [],{category,sourceId,search,onlyIds:onlyPinned ? pinnedIds : onlyChanges ? changedIds : undefined}),[situation,category,sourceId,search,onlyPinned,onlyChanges,pinnedIds,changedIds]);
  ...
  const mapQuery=useMemo(()=>({...DEFAULT_QUERY,window_hours:hours,include_inactive:true,country:scopeCountryCode(scope) || undefined,region:scope==="europe" ? "europe" as const : undefined}),[scope,hours]);
  ...
  const chooseScope=useCallback((next:ScopeId)=>{setScope(next);setLegacy(false);setOnlyChanges(false);setOnlyPinned(false);},[]);
  const toggleFavorite=useCallback((code:string)=>{
    const area=normalizeScopeId(`country:${code}`);if(!area)return;
    const safeCode=scopeCountryCode(area)!;
    const current=favoriteStorageBlocked ? favoriteCountries : readFavoriteCountries(localStore());
    const next=current.includes(safeCode) ? current.filter(value=>value!==safeCode) : [...current,safeCode];
    if(next.length>MAX_FAVORITE_COUNTRIES){setFavoriteCountries(current);setToast(`Możesz zapisać do ${MAX_FAVORITE_COUNTRIES} ulubionych obszarów. Usuń jeden, aby dodać kolejny.`);return;}
    const saved=writeFavoriteCountries(localStore(),next);setFavoriteCountries(next);setFavoriteStorageBlocked(!saved);
    setToast(!saved ? "Ulubione działają w tej karcie. Przeglądarka nie pozwala ich zapisać." : next.includes(safeCode) ? `Dodano do ulubionych: ${getScopeLabel(area)}.` : `Usunięto z ulubionych: ${getScopeLabel(area)}.`);
  },[favoriteCountries,favoriteStorageBlocked]);
  ...
  const togglePin=useCallback((id:string)=>{
    ...
    else setToast(next.includes(id) ? situation?.undatedEvents.some(event=>event.id===id) ? "Przypięto do zapisanych. Brak wymaganej daty wyklucza zapis z briefingu czasowego." : "Przypięto. Zapis znajdziesz w briefingu, jeśli mieści się w jego obszarze i czasie." : "Usunięto z przypiętych.");
  },[pinnedIds,situation]);
  ...
  return <div className={`sextet-app ${panel ? "panel-open" : ""}`}>
    ...
    <div className="sextet-body">
      <aside className="sextet-rail" aria-label="Obszar i czas">
        <div className="scope-navigation"><p className="rail-label">Obserwowany obszar</p><div className="scope-presets" role="group" aria-label="Wybierz obszar">{scopes.map((id,index)=><button key={id} aria-pressed={scope===id} onClick={()=>chooseScope(id)}><span className="scope-symbol" aria-hidden="true">{["◎","◒","PL"][index]}</span>{getScopeLabel(id)}<Icon name="arrow" size={12}/></button>)}</div><CountryPicker scope={scope} favorites={favoriteCountries} ready={sessionReady} onScopeChange={chooseScope} onToggleFavorite={toggleFavorite}/></div>
      ...
      <main id="sextet-content" className="sextet-content" tabIndex={-1}>
        ...
        {snapshot && situation && !legacy && <>
          ...
          {favoriteStorageBlocked && <p className="snapshot-caution">Ulubione obszary nie są zapisane. Działają tylko do zamknięcia lub odświeżenia tej karty.</p>}
          ...
          {view==="overview" && situation.unknownTimeCount>0 && <p className="snapshot-caution">{situation.unknownTimeCount} zapisów z tego obszaru pozostaje poza osią czasu. <button className="text-button" onClick={()=>{navigate("explore");setCategory(undefined);setSourceId(undefined);setSearch("");requestAnimationFrame(()=>{const target=document.getElementById("undated-list");target?.focus();target?.scrollIntoView({block:"start"});});}}>Pokaż zapisy bez wiarygodnej daty</button></p>}
          ...
          {view==="explore" && <>
            ...
            {sourceId && <p className="section-note">{PUBLIC_SOURCE_INFO[sourceId].name}: {coverage?.entries.find(entry=>entry.id===sourceId)?.records ?? 0} zapisów w całym odczycie źródła. Poniżej obowiązują filtry obszaru i czasu; komunikaty bez kraju znajdziesz w obszarze Świat. Starsze publikacje mogą wymagać zakresu 7 dni.</p>}
