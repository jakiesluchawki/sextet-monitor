...
import { filterViewEvents } from "@/lib/public-view";
...
export default function PublicMonitor(){
  ...
  const downloadBriefing=()=>{const blob=new Blob([briefText],{type:"text/markdown;charset=utf-8"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`sextet-briefing-${snapshot?.generated_at.slice(0,10) || "zestaw"}.md`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
