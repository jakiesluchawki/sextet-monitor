...
export default function DetailedExplorer(){
  ...
  const inspectSource=useCallback((id:string)=>{
    ...
    requestAnimationFrame(()=>{if(window.matchMedia("(max-width: 900px)").matches)document.getElementById("legacy-events-list")?.focus({preventScroll:false});});
  },[changeSource]);
  const listFallback=useCallback(()=>{setMobileView("list");requestAnimationFrame(()=>document.getElementById("legacy-events-list")?.focus({preventScroll:false}));},[]);
  useEffect(()=>{
  ...
  return <div className="monitor-app">
    ...
    <main className="workspace">
      ...
      <section className="center-workspace" aria-label="Mapa i wyniki zestawu">
        ...
        <section id="legacy-events-list" className={`results-section ${mobileView==="list" ? "mobile-visible" : ""}`} aria-label="Lista i oś czasu" tabIndex={-1}>
      ...
      <aside id="legacy-details-panel" ref={evidencePanel} className="evidence-workspace" aria-label="Dowody i źródła zestawu" tabIndex={-1}><nav className="evidence-tabs" aria-label="Panel szczegółów">{([["evidence","Dowody"],["sources","Źródła"]] as const).map(([key,label])=><button key={key} aria-pressed={rightTab===key} onClick={()=>{setRightTab(key);setRevealRequest(null);}}>{label}</button>)}</nav><div className="evidence-scroll">
